...
<#
.SYNOPSIS
Installs the latest Windows  quality updates.
.DESCRIPTION
...
# Start logging
Start-Transcript "$($env:SystemRoot)\Logs\Software\UpdateOS.log"
...
Get-WindowsUpdate -Install -IgnoreUserInput -AcceptAll -WindowsUpdate -IgnoreReboot -Verbose | Select Title, KB, Result | Format-Table
Get-WindowsUpdate -Install -IgnoreUserInput -AcceptAll -MicrosoftUpdate -IgnoreReboot -Verbose | Select Title, KB, Result | Format-Table
